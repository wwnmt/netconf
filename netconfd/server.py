...
from netconfd import base
import netconfd.error as ncerror
from netconfd import NSMAP
from netconfd import qmap
        ...
        for rpc in rpcs:
            ...
            try:
                ...
                if rpcname == "close-session":
                    ...
                    config_param = rpc_method.find("nc:config", namespaces=NSMAP)
                    if config_param is None:
                        raise ncerror.MissingElementProtoError(rpc, util,qname("nc:config"))
                    params = [target_param, config_param]

                elif rpcname == "copy-config":
                    ...
                    if source_param is None:
                        raise ncerror.MissingElementProtoError(rpc, util.qname("nc:source"))
                    params = [target_param, source_param] 

                elif rpcname == "delete-config" or rpcname == "Lock" or rpcname == "unLock":
                ...
                elif rpcname == 'create-subscription':

                    if paramslen > 1:
                        raise ncerror.MalformedMessageRPCError(rpc)
                    stream_param = rpc_method.find("nc:stream", namespaces=NSMAP)
                    if stream_param is None:
                        raise ncerror.MissingElementProtoError(rpc, util.qname("nc:stream"))
                    params = [stream_param, ]                   
...
class NetconfMethods(object):
    ...
    def rpc_create_subscription(self, unused_session, rpc, stream, *unused_params):
